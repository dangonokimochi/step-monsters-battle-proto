...
簡易スーパーロボット大戦に近いグリッドSRPGを、8×8・4vs4に縮小してSPD順ターン制にしたもの。
...
- 座標系：col(0-7, 左→右), row(0-7, 上→下)

### 初期配置
- 味方：col 0〜3（左半分）に自由配置
- 敵：col 4〜7（右半分）に自動配置
- 1マスに1体まで

### 地形
戦闘ごとにランダムで配置。密度はバランス調整で決定（目安：8〜12マス程度に地形を配置）。
...
| 鬼火弾 | 3 | false | 30% | 8 | 1.3 |
| 怨念の炎 | 4 | true | 50% | 15 | 1.5 |
| 魂の灯 | 3 | true | 0% | 10 | 0.0（味方HP20回復） |
